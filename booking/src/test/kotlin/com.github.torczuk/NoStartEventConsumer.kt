...
class NoStartEventConsumer {
    ...
    fun kafkaBookingEventConsumer(bookingEventListener: EventListener<BookingEvent>,
                                  ...
                                  threadPoolTaskExecutor: ThreadPoolTaskExecutor) = KafkaEventConsumer(bookingEventListener, ConsumerConfiguration().properties(), objectMapper, "booking_events", BookingEvent::class.java)

    @Bean
    fun kafkaOrderEventConsumer(orderEventListener: EventListener<OrderEvent>,
                                objectMapper: ObjectMapper,
                                threadPoolTaskExecutor: ThreadPoolTaskExecutor) = KafkaEventConsumer(orderEventListener, ConsumerConfiguration().properties(), objectMapper, "order_events", OrderEvent::class.java)

    @Bean
    fun kafkaPaymentEventConsumer(paymentEventListener: EventListener<PaymentEvent>,
                                  objectMapper: ObjectMapper,
                                  threadPoolTaskExecutor: ThreadPoolTaskExecutor) = KafkaEventConsumer(paymentEventListener, ConsumerConfiguration().properties(), objectMapper, "payment_events", PaymentEvent::class.java)
